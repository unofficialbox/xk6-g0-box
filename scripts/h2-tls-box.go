...
func Setup(ctx context.Context, assert *require.Assertions) interface{} {
	...
	accessToken := GetAccessToken(client, assert, cfg)
	folderId := CreateBaseFolder(client, assert, accessToken, baseFolderId)
...
func Default(ctx context.Context, assert *require.Assertions, data interface{}) {
	...
	cusr := CreateUploadSession(client, assert, fileName, fileSize, folderId, accessToken)
	log.Printf("Found create upload session res: %+v", cusr)

	parts := UploadParts(client, ctx, assert, cusr, filePath, fileSize, accessToken)
	...
	CommitUploadSession(client, assert, accessToken, fileHash, cusr.SessionEndpoints.Commit, commitReq)
}

func GetAccessToken(client *http.Client, assert *require.Assertions, cfg Config) string {
...
func CreateBaseFolder(client *http.Client, assert *require.Assertions, accessToken string, baseFolderId string) string {
...
func CreateUploadSession(client *http.Client, assert *require.Assertions, fileName string, fileSize int, folderId string, accessToken string) CreateUploadSessionResponse {
...
func UploadParts(client *http.Client, ctx context.Context, assert *require.Assertions, usr CreateUploadSessionResponse, filePath string, fileSize int, accessToken string) []Part {
	...
	for i := 0; i < totalParts; i++ {
		...
		go UploadSinglePart(client, assert, usr, accessToken, sha1_hash, chunk, fileSize, buffer, partchan)
		part := <-partchan
...
func UploadSinglePart(client *http.Client, assert *require.Assertions, usr CreateUploadSessionResponse, accessToken string, sha1_hash string, chunk Chunk, fileSize int, buffer []byte, partchan chan<- Part) {
...
func CommitUploadSession(client *http.Client, assert *require.Assertions, accessToken string, fileHash string, commitURL string, cusr CommitUploadSessionRequest) string {
	fmt.Println("")
